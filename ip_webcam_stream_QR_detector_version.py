...
# Initialize QR Code Detector (globally for efficiency)
...
def generate_frames():
    ...
    Includes QR code detection logic.
    ...
    while True:
        ...
        if points is not None:
            ...
            # points is a numpy array of shape (1, 4, 2) for the 4 corners of the QR code
            points = points[0].astype(int) # Convert to integer coordinates
            
            # Draw lines connecting the 4 corners
            cv2.polylines(frame, [points], True, (0, 255, 0), 2) # Green color (BGR)
            ...
            if decoded_info:
                ...
                cv2.putText(frame, "QR Code Detected (Undecodable)", (points[0][0], points[0][1] - 10), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 165, 255), 2, cv2.LINE_AA) # Orange for undecodable
                
            # Add overall "QR Code Detected!" message at a fixed position
            cv2.putText(frame, "QR Code Detected!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA) # Green text (BGR)
...
<body class="bg-gradient-to-br from-blue-100 to-purple-100 flex items-center justify-center min-h-screen p-4 sm:p-6 md:p-8">
    <div class="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-lg w-full text-center border-t-8 border-blue-600 transform hover:scale-105 transition-transform duration-300">
        ...
        <p class="text-sm text-gray-600 mt-2 leading-relaxed">
            <span class="font-semibold text-green-600">QR Code Detection Active</span> (Detected QR codes will be outlined in green with decoded data.)
        </p>
