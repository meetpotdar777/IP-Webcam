...
# Global variable for face cascade classifier
# It's important to load this once globally to avoid repeated disk access
try:
    face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
    if face_cascade.empty():
        print("WARNING: Could not load Haar cascade classifier for face detection. Face detection will be disabled.")
        face_cascade = None
except Exception as e:
    print(f"ERROR: Failed to load Haar cascade classifier: {e}. Face detection will be disabled.")
    face_cascade = None
...
def generate_frames():
    ...
    while True:
        ...
        face_detected = False
        if face_cascade: # Check if cascade classifier was loaded successfully
            ...
            for (x, y, w, h) in faces:
                cv2.rectangle(frame, (x, y), (x+w, y+h), (255, 0, 0), 2) # Blue rectangle for faces (BGR)

            # Add "Face Detected!" text if faces are found
            if face_detected:
                cv2.putText(frame, "FACE DETECTED!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2, cv2.LINE_AA) # Blue text (BGR)
...
<head>
    ...
    <style>
        ...
        .aspect-video img { /* Only img element will be used for stream display */
            ...
            /* Filters are removed, so no transition needed here */
        }
...
<body class="bg-gradient-to-br from-blue-100 to-purple-100 flex items-center justify-center min-h-screen p-4 sm:p-6 md:p-8">
    <div class="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-lg w-full text-center border-t-8 border-blue-600 transform hover:scale-105 transition-transform duration-300">
        ...
        <p class="text-sm text-gray-600 mt-2 leading-relaxed">
            <span class="font-semibold text-blue-700">Face Detection Active</span> (Faces will be outlined in blue on the stream.)
        </p>
