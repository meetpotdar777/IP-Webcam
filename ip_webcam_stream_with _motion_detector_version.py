...
# Global variable for previous frame in motion detection
prev_frame_gray = None
# Motion detection parameters
MIN_MOTION_AREA = 1500 # Minimum contour area for motion detection (adjust as needed)
MOTION_THRESHOLD_DELTA = 25 # Threshold for image difference
GAUSSIAN_BLUR_SIZE = (21, 21) # Blur kernel size (must be odd and positive)
...
def release_camera():
    ...
    """
    global camera
    global prev_frame_gray # Also reset prev_frame_gray when camera is released
    with camera_lock:
...
def generate_frames():
    ...
    Includes motion detection logic.
    If the camera is not available, it serves a placeholder image.
    """
    global prev_frame_gray # Access the global variable for motion detection
    ...
    while True:
        if camera_instance is None or not camera_instance.isOpened():
            ...
            camera_instance = get_camera()
            prev_frame_gray = None # Reset prev_frame if camera was reconnected/failed
        ...
        if not success:
            ...
            prev_frame_gray = None # Reset prev_frame on camera error
            continue # Go back to the start of the loop

        # Convert frame to grayscale for motion detection
        gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        gray_frame = cv2.GaussianBlur(gray_frame, GAUSSIAN_BLUR_SIZE, 0)
        ...
        if prev_frame_gray is not None:
            ...
            thresh = cv2.threshold(frame_delta, MOTION_THRESHOLD_DELTA, 255, cv2.THRESH_BINARY)[1]
            ...
            for contour in contours:
                if cv2.contourArea(contour) < MIN_MOTION_AREA:
                    continue
                # Motion detected: draw bounding box
                (x, y, w, h) = cv2.boundingRect(contour)
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2) # Green rectangle (BGR)
                motion_detected = True

        # Update previous frame for the next iteration
        prev_frame_gray = gray_frame.copy()

        # Add "Motion Detected!" text if motion is found
        if motion_detected:
            cv2.putText(frame, "MOTION DETECTED!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA) # Red text (BGR)
...
<body class="bg-gradient-to-br from-blue-100 to-purple-100 flex items-center justify-center min-h-screen p-4 sm:p-6 md:p-8">
    <div class="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-lg w-full text-center border-t-8 border-blue-600 transform hover:scale-105 transition-transform duration-300">
        ...
        <p class="text-sm text-gray-600 mt-2 leading-relaxed">
            <span class="font-semibold text-green-700">Motion Detection Active</span> (Applies to IP stream only. Detected motion will be highlighted in green.)
        </p>
...
if __name__ == '__main__':
    ...
    # No longer call get_camera() here directly at startup for userMedia,
    # but the generate_frames will attempt to get it when Flask stream is active.
