...
def generate_frames():
    ...
    while True:
        ...
        # Motion detection logic removed from here
...
<head>
    ...
    <style>
        ...
        .aspect-video img { /* Only img element will be used for stream display */
            ...
            transition: filter 0.3s ease-in-out; /* Smooth transition for filters */
        }
...
<body class="bg-gradient-to-br from-blue-100 to-purple-100 flex items-center justify-center min-h-screen p-4 sm:p-6 md:p-8">
    <div class="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-lg w-full text-center border-t-8 border-blue-600 transform hover:scale-105 transition-transform duration-300">
        ...
        <div id="videoContainer" class="relative w-full overflow-hidden rounded-lg border-4 border-gray-300 bg-black flex items-center justify-center aspect-video">
            ...
            <!-- Hidden Canvas for Snapshots with Filters -->
        ...
        <div class="mt-6 flex flex-col gap-4 justify-center">
            ...
            <div class="flex flex-wrap gap-3 justify-center">
                <button id="normalFilterBtn" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-full shadow-lg transition-all duration-300 transform hover:scale-105 active:scale-95">
                    ✨ Normal
                </button>
                <button id="grayscaleFilterBtn" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-full shadow-lg transition-all duration-300 transform hover:scale-105 active:scale-95">
                    ⚫ Grayscale
                </button>
                <button id="sepiaFilterBtn" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-full shadow-lg transition-all duration-300 transform hover:scale-105 active:scale-95">
                    🤎 Sepia
                </button>
            </div>
            <!-- Existing: Take Snapshot Button -->
    ...
    <script>
        ...
        const normalFilterBtn = document.getElementById('normalFilterBtn');
        ...
        let currentFilter = 'none'; // To track active filter
        ...
        function applyFilter(filterType) {
            ...
            if (filterType === 'grayscale') {
                flaskVideoStream.style.filter = 'grayscale(100%)';
                showToast('Grayscale filter applied!', 'info');
            } else if (filterType === 'sepia') {
                flaskVideoStream.style.filter = 'sepia(100%)';
                showToast('Sepia filter applied!', 'info');
            } else { // 'none'
                flaskVideoStream.style.filter = 'none';
                showToast('Normal filter applied!', 'info');
            }
        }
        ...
        if (takeSnapshotBtn) {
            takeSnapshotBtn.addEventListener('click', function() {
                ...
                // Apply the current CSS filter to the canvas context
                // This ensures the drawn image on canvas includes the visual filter
                context.filter = flaskVideoStream.style.filter; // Use the same filter applied to the img

                // Draw the current video frame onto the canvas
                context.drawImage(flaskVideoStream, 0, 0, hiddenCanvas.width, hiddenCanvas.height);

                // Reset filter on context to avoid affecting subsequent draws
                context.filter = 'none';
        ...
        if (normalFilterBtn) {
            normalFilterBtn.addEventListener('click', () => applyFilter('none'));
        }
        if (grayscaleFilterBtn) {
